...
<div class="terminal-header">
    <h1>{emojis['target']} DEEP ANALYSIS TERMINAL {emojis['target']}</h1>
    <p>{emojis['brain']} Statistical Analysis & Pattern Detection System</p>
</div>
...
if not st.session_state.current_dataset:
    ...
    <div class="terminal-block" style="border-color: #ff0000;">
        ...
        <p style="color: #00ffff;">Upload a dataset first to initialize analysis terminal</p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
# Sidebar
with st.sidebar:
    st.markdown(f"### {emojis['brain']} ANALYSIS CONFIGURATION")
    ...
    st.markdown("---")
    st.markdown(f"{emojis['data']} **Dataset:** {st.session_state.current_dataset}")
    st.markdown(f"{emojis['target']} **Records:** {len(df):,}")
    st.markdown(f"{emojis['gear']} **Fields:** {len(df.columns)}")
