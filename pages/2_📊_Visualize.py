...
<div class="terminal-header">
    <h1>{emojis['chart']} VISUAL ANALYTICS TERMINAL {emojis['chart']}</h1>
    <p>{emojis['fire']} Advanced Data Visualization & Pattern Recognition</p>
</div>
...
if not st.session_state.current_dataset:
    ...
    <div class="terminal-block" style="border-color: #ff0000;">
        ...
        <p style="color: #00ffff;">Upload a dataset first to initialize visualization terminal</p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with st.sidebar:
    st.markdown(f"### {emojis['gear']} CHART CONFIGURATION")
    ...
    if st.button("Generate Chart", type="primary", key="pie_gen"):
        ...
        if fig:
            ...
            with col_d2:
                try:
                    img_bytes = fig.to_image(format="png", width=1200, height=800)
                    st.download_button("🖼️ Download PNG", img_bytes, f"{chart_type.replace(' ', '_')}.png", "image/png", use_container_width=True)
                except:
